 ...
 *
 * This program was derived from Markku Rossi work.
 *
...
void
Twitter::set_twitter_endpoint(const prog_char server[], const prog_char uri[], uint16_t port)
{
  this->server = server;
  this->uri = uri;
  this->port = port;
...
Twitter::query_time(void)
{
  ...
  if (!http.sslConnect(server, port))
    {
...
Twitter::parse_month(char *str, char **end)
{
  ...
  *end = cp;
  ...
  for (i = 0; i < 12; i++)
    if (strcmp_P(str, (char *) pgm_read_dword(&(months[i]))) == 0)
      return i + 1;
...
Twitter::post_status(const char *message)
{
  ...
  url_encode_pgm(buffer, access_token);
...
Twitter::compute_authorization(const char *message)
{
  ...
  *cp++ = '&';
  ...
  cp = url_encode_pgm(cp, token_secret);
